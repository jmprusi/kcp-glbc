...
func Services(t Test, namespace *corev1.Namespace, labelSelector string) func(g gomega.Gomega) []corev1.Service {
	return func(g gomega.Gomega) []corev1.Service {
		services, err := t.Client().Core().Cluster(logicalcluster.From(namespace).Path()).CoreV1().Services(namespace.Name).List(t.Ctx(), metav1.ListOptions{LabelSelector: labelSelector})
